function [p, h, stats] = wilcoxon(x,y,varargin)
...
pos = find(diffxy>0);
victPos = length(pos);
[tie_rank, tieadj] = tiedrank(abs(diffxy));
...
if nargout > 1
    h = (p<=alpha);
    if (nargout > 2)
        if (sum(tie_rank(neg))>sum(tie_rank(pos)))
            y = y + victNeg*empates + sum(1:empates)/2;
            w = w + victPos*empates + sum(1:empates)/2;
            stats.signedrankAlg1 = w;
            stats.signedrankAlg2 = y;
        else
            y = y + victPos*empates + sum(1:empates)/2;
            w = w + victNeg*empates + sum(1:empates)/2;
            stats.signedrankAlg1 = y;
            stats.signedrankAlg2 = w;
        end
    end
end