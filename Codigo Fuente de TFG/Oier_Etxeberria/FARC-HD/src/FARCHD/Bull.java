...
public class Bull {
  ...
  double minsup, minconf, alpha, porApr, porTest;
  ...
  public void execute() {
    if (this.somethingWrong) { //We do not execute the program
      ...
      //We should not use the statement: System.exit(-1);
        ...
        totalTimeApriori = System.currentTimeMillis() - startTimeApriori;
        System.out.println("Tiempo de construcción de apriori " + totalTimeApriori);
        totalTimeApriori /= 1000;
        long seg = totalTimeApriori % 60;
        totalTimeApriori /= 60;
        long min = totalTimeApriori % 60;
        long hor = totalTimeApriori / 60;
        String stringApriori = "";

        if (hor < 10)  stringApriori = stringApriori + "0"+ hor + ":";
        else   stringApriori = stringApriori + hor + ":";

        if (min < 10)  stringApriori = stringApriori + "0"+ min + ":";
        else   stringApriori = stringApriori + min + ":";

        if (seg < 10)  stringApriori = stringApriori + "0"+ seg;
        else   stringApriori = stringApriori + seg;

        System.out.println("Tiempo de construcción de apriori " + stringApriori);
        ...
        totalTimeConstruction = System.currentTimeMillis() - startTimeApriori;
        ...
        startTimeEvTest = System.currentTimeMillis();
        //int delay = 2000; // number of milliseconds to sleep
        //long start = System.currentTimeMillis();
        //while(start >= System.currentTimeMillis() - delay); // do nothing
        doOutput(this.test, this.outputTst);
        totalTimeEvTest = System.currentTimeMillis() - startTimeEvTest;
	  
        totalTime = System.currentTimeMillis() - startTime;
        porApr = ((double)(totalTime-totalTimeEvTest)/(double)totalTime)*100.0;
        porTest = ((double)totalTimeEvTest/(double)totalTime)*100.0;

        System.out.println("Teimpos, total: " + totalTime + " cons: " + (totalTime-totalTimeEvTest) + " test: " + totalTimeEvTest + " Porcentajes de tiempo, construcción: " + porApr + " test: " + porTest);
        this.writeTime();
        this.writeRules();
        this.writeEvo();
  ...
  public void writeTime() {
    long aux, seg, min, hor, tT, tC, tE;
    String stringOut = new String("");
  
    tT = totalTime % 1000;
    tC = totalTimeConstruction % 1000;
    tE = totalTimeEvTest % 1000;
    stringOut = "" + totalTime / 1000 + ":" + tT +  " " + totalTimeConstruction / 1000 + ":" + tC +  " " + totalTimeEvTest / 1000 + ":" +  tE +  " " + data + "\n";
